...
2. **Ejecuta el analisis determinista** (cero tokens, todo el trabajo lo hace el script):
   ```
   python "scripts/auditar_flujo.py" "<ruta al .zip / carpeta / definition.json>"
   ```
...
3. **Presenta el resultado en Markdown en el chat**, en este orden:
   ...
   - Cargas condicionales (no cargues todo de una vez):
     ...
     | Lee | Solo cuando |
     |---|---|
     | `references/buenas-practicas.md` — con grep del codigo `PA-XXX-NN`, nunca entero | expliques el porque/contexto de una regla concreta |
     | `references/ia-en-flujos.md` | el reporte traiga hallazgos PA-IA-xx / PA-AGT-xx |
...
- El catalogo de reglas y sus fuentes se mantiene en
  `references/buenas-practicas.md` (lleva fecha y enlaces a
